const socketMiddleware = store => {
  ...
  const onUpdateClient = message => {
    ...
    const recipientLost = (messageState.recipient !== UI.NO_RECIPIENT) &&
      !(message.list.find(user => user === messageState.recipient))

    // Has our previously disconnected recipient reconnected?
    const recipientFound = !!messageState.lostRecipient &&
      !!message.list.find(user => user === messageState.lostRecipient)
